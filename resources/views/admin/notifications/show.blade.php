    ...
    @else
        <div class="rounded-3xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-slate-950 shadow-sm overflow-hidden">
            <div class="p-6">
                ...
                @if(data_get($data, 'link') && !isset($isDeleted))
                    <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800">
                        <a href="{{ data_get($data, 'link') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition">
                            <i class="bi bi-box-arrow-up-right"></i>
                            View {{ data_get($data, 'deleted_type', ucfirst(data_get($data, 'type', 'item'))) }}
                        </a>
                    </div>
                @endif
            </div>
