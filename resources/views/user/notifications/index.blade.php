    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                @if($unreadCount > 0)
                <form method="POST" action="{{ route('user.notifications.mark-all-read') }}" class="inline" id="markAllReadForm">
                    @csrf
                    <button type="submit" 
                ...
                @if($readCount > 0)
                <form method="POST" action="{{ route('user.notifications.delete-all') }}" class="inline" id="deleteAllForm">
                    @csrf
                    <button type="submit" 
    ...
    <div class="space-y-2">
        @forelse($notifications as $notification)
            @php
                ...
                elseif (str_contains($type, 'Ticket') || str_contains($type, 'ticket') || str_contains($type, 'TicketCreated')) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    setTimeout(() => {
                        ...
                        if (data.is_deleted) {
                            // For deleted notifications, redirect to notification show page
                            window.location.href = data.notification_url || targetUrl;
                        } else if (data.redirect_url) {
                            // For normal notifications, redirect to the resource
